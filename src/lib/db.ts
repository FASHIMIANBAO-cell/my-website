...
export const post = {
  ...
  update: async (id: number, data: Partial<{ slug: string; title: string; content: string; excerpt: string; published: boolean }>) => {
    const s = await getSql();
    await s`UPDATE Post SET slug = ${data.slug ?? ''}, title = ${data.title ?? ''}, content = ${data.content ?? ''}, excerpt = ${data.excerpt ?? ''}, published = ${data.published ? 1 : 0}, "updatedAt" = NOW() WHERE id = ${id}`;
...
export const resource = {
  ...
  update: async (id: number, data: Partial<{ name: string; description: string; category: string; image: string; downloadLinks: string }>) => {
    const s = await getSql();
    await s`UPDATE Resource SET name = ${data.name}, description = ${data.description ?? ''}, category = ${data.category ?? ''}, image = ${data.image ?? ''}, "downloadLinks" = ${data.downloadLinks ?? '[]'} WHERE id = ${id}`;
...
export const comment = {
  list: async (resourceId?: number | null) => {
    ...
    let r;
    if (resourceId === null) {
      r = await s`SELECT c.id, c."parentId", c."resourceId", c.content, c."createdAt",
